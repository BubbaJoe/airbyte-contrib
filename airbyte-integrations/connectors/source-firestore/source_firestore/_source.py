...
class SourceFirestore(Source):
    def check(self, logger: AirbyteLogger, config: json) -> AirbyteConnectionStatus:
        ...
        try:
            connection = {}
            connection["project"] = config['project_id']

            if config['credentials_json']:
                try:
                    json_account_info = json.loads(config['credentials_json'])
                except ValueError:
    ...
    def read(
        ...
        logger.info(f"Reading data from {len(catalog.streams)} Firestore tables.")
        with establish_connection(config, logger) as connection:
